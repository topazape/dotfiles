keyring_name=docker-archive-keyring.gpg codename=$(. /etc/os-release;echo $VERSION_CODENAME) \
	...
	&& echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/$keyring_name] https://download.docker.com/linux/debian $codename stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null \
